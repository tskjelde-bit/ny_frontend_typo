  ...
  return (
    <aside className={`bg-[#1e293b] flex flex-col h-full overflow-hidden p-6 md:p-8 ${className}`}>
      ...
      <div className="flex items-center justify-between mb-8 shrink-0">
        <h3 className="text-[0.6875rem] font-semibold uppercase tracking-[0.08em] text-slate-500">Siste innlegg</h3>
        <button className="text-[0.6875rem] font-semibold text-blue-500 hover:text-blue-400 transition-colors uppercase tracking-[0.08em] flex items-center gap-1">
        ...
        <div className="space-y-8">
          {posts.map((post, idx) => (
            <div key={idx} className="group cursor-pointer">
              {post.featured && post.image && (
                <div className="relative aspect-[16/10] rounded-[1rem] overflow-hidden mb-5 bg-slate-900 border border-white/5">
                  ...
                  <div className="absolute top-4 right-4">
                    <span className="bg-blue-600 text-white text-[0.6875rem] font-semibold px-3 py-1 rounded-lg tracking-[0.08em] uppercase shadow-xl">
                      {post.category}
                    </span>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-2 mb-2 text-[0.625rem] font-semibold text-slate-500 tracking-[0.06em] uppercase">
                <span>{post.date}</span>
                <span className="opacity-30">•</span>
                <span>{post.category}</span>
              </div>
              
              <h4 className="text-slate-200 font-bold leading-[1.3] tracking-[-0.01em] group-hover:text-blue-400 transition-colors text-[1.375rem]">
                {post.title}
              </h4>
      ...
      <div className="mt-8 pt-8 border-t border-white/10 shrink-0">
        <h3 className="text-slate-200 text-[1.0625rem] font-semibold leading-snug mb-3">
          Motta min månedlige oppdatering på boligmarkedet i Oslo.
        </h3>
        <p className="text-slate-400 text-[0.9375rem] font-normal mb-6 leading-relaxed italic opacity-80">
        ...
        <form onSubmit={handleSubmit} className="relative">
          <div className={`flex items-center border-b transition-all duration-300 pb-2 ${isFocused ? 'border-blue-500' : 'border-white/20'}`}>
            <input 
              ...
              className="bg-transparent border-none outline-none text-white placeholder:text-slate-500 font-normal text-[0.875rem] w-full py-1"
            />
            <button 
              type="submit"
        ...
        <p className="mt-6 text-[0.625rem] text-slate-500 font-semibold uppercase tracking-[0.06em]">Avmeld når som helst</p>
      </div>
    </aside>
  );
};
